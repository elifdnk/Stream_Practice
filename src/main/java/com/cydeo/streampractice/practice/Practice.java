...
public class Practice {
    ...
    public static List<String> getAllDepartmentManagerFirstNames() {
        return departmentService.readAll().stream().map(department -> department.getManager().getFirstName()).collect(Collectors.toList());
    }

    //11 Display all the departments where manager name of the department is 'Steven'
    public static List<Department> getAllDepartmentsWhichManagerFirstNameIsSteven() {
        return departmentService.readAll().stream().filter(department -> department.getManager().getFirstName().equals("Steven")).collect(Collectors.toList());
    }

    //12 Display all the departments where postal code of the location of the department is '98199'
    public static List<Department> getAllDepartmentsWhereLocationPostalCodeIs98199() {
        return departmentService.readAll().stream().filter(department -> department.getLocation().getPostalCode().equals("98199")).collect(Collectors.toList());
    }
...
//                .get().getSalary();
    ...
    public static List<Employee> getMaxSalaryEmployee() {
        ...
        return employeeService.readAll().stream()
                .filter(employee -> employee.getSalary() == a)
                .collect(Collectors.toList());
...
//        return (long) departmentService.readAll().size();
    ...
    public static Employee getEmployeeWhoseFirstNameIsAlyssaAndManagersFirstNameIsEleniAndDepartmentNameIsSales() throws Exception {
        ...
        return employeeService.readAll().stream()
                .filter(employee -> employee.getFirstName().equalsIgnoreCase("Alyssa")
                        &&
                        employee.getManager().getFirstName().equalsIgnoreCase("Eleni") && employee.getDepartment().getDepartmentName().equalsIgnoreCase("Sales")).findFirst().get();
    }
    ...
    public static List<JobHistory> getAllJobHistoriesInDescendingOrder() {
        return jobHistoryService.readAll().stream()
                .sorted(Comparator.comparing(JobHistory::getStartDate).reversed()).collect(Collectors.toList());
    }

    //38 Display all the job histories where the start date is after 01.01.2005
    public static List<JobHistory> getAllJobHistoriesStartDateAfterFirstDayOfJanuary2005() {
        LocalDate startDateAfter = LocalDate.of(2005, 1, 1);

        return jobHistoryService.readAll().stream()
                .filter(date -> date.getStartDate().isAfter(startDateAfter)).collect(Collectors.toList());
    }
    ...
    public static List<JobHistory> getAllJobHistoriesEndDateIsLastDayOfDecember2007AndJobTitleIsProgrammer() {
        ...
        return jobHistoryService.readAll().stream()
                .filter(p -> p.getEndDate().equals(endDate) && p.getJob().getJobTitle().equalsIgnoreCase("Programmer")).collect(Collectors.toList());
    }
    ...
    public static Employee getEmployeeOfJobHistoryWhoseStartDateIsFirstDayOfJanuary2007AndEndDateIsLastDayOfDecember2007AndDepartmentNameIsShipping() throws Exception {
        ...
        return jobHistoryService.readAll().stream()
                .filter(p -> p.getStartDate().equals(LocalDate.of(2007, 01, 01)) && p.getEndDate().equals(LocalDate.of(2007, 12, 31)) && p.getDepartment().getDepartmentName().equalsIgnoreCase("Shipping"))
...
//        return initialsList;
    ...
    public static List<Employee> getLongestNamedEmployee() {
        return employeeService.readAll().stream()
                .filter(employee ->
                        employee.getFirstName().length() + employee.getLastName().length() + 1 == getLongestNameLength()) //+1 is space length
                .collect(Collectors.toList());
    }
